...
- **One exception to "configuration files are never rewritten by the plugin"**: the `/events` setup commands (`create|claim|setzone|delzone|setblock|delete`) and `/schedule add|remove` (`hcfcore.events.admin`, `events/setup/EventSetup`, `events/planning/ScheduleCommand`) write into `events.yml` for staff who set up an event in game rather than by hand. A new event is the jar's own example of its kind (`EventTemplate`, pure), moved to the staff member; `EventKind` says what each kind has to set up. Only the one section of the one event they name is touched — `YamlConfiguration` with `options().parseComments(true)` (Paper's own default, kept explicit), so the rest of the file, its comments and the documentation's `--8<--` markers survive. The write happens on the main thread, like the command that triggers it: the file is a few kilobytes and these commands are typed rarely enough that an async round trip would not be worth the complexity.
...
- **Totem** (`events/totem/`, `TotemManager`) is a sixth engine (the owner's event of 22/09/2026): a column of blocks broken one by one by one team, **started over whenever another team breaks one** (`RivalBreak`). The column is drawn from the run - active, broken, or idle between runs - never edited block by block, so a reset, a win or a stop all come back to the same picture; the break is recorded at `MONITOR` like the DTC's, and, with `instant-break` on (off as shipped: a block is mined the whole way), `BlockDamageEvent#setInstaBreak` makes an allowed sword hit break at once. `BreakAllowance` opens the running column's unbroken blocks through territory protection, alongside the running core.
- **Setup is common, engines are not** (`events/setup/`, the owner's choice of 22/09/2026): the six engines share no method, but staff set every one of them up with the same verbs. `EventSetup` works on `events.yml` itself, not on the engines' definitions - an event that failed to load can still be looked at, fixed or deleted - and asks each engine only whether an id runs. An event's **territory** is a server team's claims, as for any server land: the claim module draws it (`TeamClaimTask`), `events/` only names the team in the `claim` key, and `/events delete` disbands it, which releases the land through the claim module's own disband listener.
- **One launcher** (`events/EventLauncher`): starting or stopping an event by id, whatever its engine, lives in one place, so `/events start` and the weekly schedule (`events/planning/PlanningController`) cannot drift apart. The schedule's rules - which starts fall in a tick's window, the week ahead - are pure (`WeeklySchedule`, `WeekAgenda`).
- **Announcements have one exit** (`util/Announcements`): every server-wide broadcast - an event's, the King's, a map phase's, a bounty's - hands its language key and text to a single listener, a seam with a neutral default. The Discord integration is that listener today; routing on language keys means a new announcement reaches Discord with no code of its own, and an operator decides what goes where in `discord.yml`.
...
| Team permissions (`TeamPermission`) | `team/` | only `/team`'s own actions in `/team settings` | `claim/` (claim, unclaim, set home, lockclaim, open every subclaim: each role check asks `TeamManager#requiredRole`, so a team's choice counts) |
| Team settings buttons (`TeamMenuButton`) | `team/` | only the window's own buttons | `claim/` (the claim lock, set HQ, set base: each runs its `/team` command as the player) |
