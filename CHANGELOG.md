...
### Added
- **Safe zones take no damage at all** (`pvp.yml`, `safe-zones.no-damage`) and **keep players fed** (`keep-fed`), both on as shipped: on spawn, a fall, fire, drowning, suffocation, a mob or a cactus does nothing, and hunger stays full. *Existing servers: add `no-damage: true` and `keep-fed: true` under `safe-zones` — without them, both defaults (on) apply.*
