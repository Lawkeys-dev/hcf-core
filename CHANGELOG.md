...
### Added
- **No abilities on an event's land** (`abilities.yml`, `disabled-in.event-territory`, on as shipped): partner items are refused on the territory of every event — its server team's claims — at all times, running or not, as on a safe zone. An event decides for its own land with `disable-abilities: true|false` in `events.yml`. *Existing servers: add `event-territory: true` under `disabled-in` — without it, the default (on) applies.*
