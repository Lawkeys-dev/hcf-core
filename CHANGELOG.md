...
### Added
- **Totem and Mini Totem** (`events.yml`, `totem:`): a column of 5 blocks (3 for the Mini Totem), bedrock between runs and quartz during one. Teams break it with a sword, one hit a block; every block a team breaks turns to bedrock, and the first team to break the whole column wins — but **a block broken by any other team starts it over**, every block quartz again (`rival-break`, `RESET` or `RESET_AND_START`). Set up with `/events create totem|minitotem <id>` and `/events settotem <id>`; `%totem_line%` on the scoreboard, a hologram, a Lunar waypoint and `points.per-totem-win` in `teams.yml`. *Existing servers: copy the `totem:` section of `events.yml`, `per-totem-win` of `teams.yml` and the `%totem_line%` rows of `ui.yml` from the jar.*
