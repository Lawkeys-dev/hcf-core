...
- **A shop** (`economy.yml`, `shop`): `[Buy]` and `[Sell]` signs placed by staff, the `/shop` menu, or both (`mode`). Ships a 21-item catalogue priced on a 10-a-kill economy — ores sold; pearls, golden apples, XP, food, potion ingredients and blocks bought; only plain items are ever taken.
