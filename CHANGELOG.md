...
### Added
- **No abilities on an event's land while it runs** (`abilities.yml`, `disabled-in.event-territory`, on as shipped): partner items are refused on the territory of a running event — its server team's claims, not only the zone held or stood in — until it ends. An event decides for its own land with `disable-abilities: true|false` in `events.yml`. *Existing servers: add `event-territory: true` under `disabled-in` — without it, the default (on) applies.*
