...
### Changed
- **Team points have a scale out of the box** (`teams.yml`), where everything shipped at `0`: **a kill +1, a death −2**, and each event worth about as many kills as the fight it takes — a KOTH capture `10`, a Citadel `30`, a Conquest `25`, a DTC and a Slide `20`, Kill the King, a Last Break and a Totem `15`, a Mini Totem `8`. A Citadel and a Mini Totem now have values of their own (`points.per-citadel-capture`, `per-mini-totem-win`) instead of a KOTH's and a Totem's. *Existing files keep their own numbers; take the `points` section from the jar for the new scale.*
