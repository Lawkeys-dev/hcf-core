...
### Added
- **`/team map` is drawn in the world** (`claims.yml`, `map.style`, `pillars` as shipped): a column on each corner of every claim around you, one kind of block per team — drawn at random each time, named in the chat under the map — shown to you alone and gone after 20 seconds. `/team map chat` still draws the old grid, whose size is now configurable too (`map.chat.radius-x`, `radius-z`). *Existing servers: `map.cell-blocks` moved to `map.chat.cell-blocks`; take the new `map` section from the jar.*
- **A locked claim shows a wall** (`claims.yml`, `lock.wall`): red glass around its border, to the players the lock refuses and to nobody else, sent like the wand's columns and never placed. Members see nothing.
