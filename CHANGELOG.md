...
- **Totem and Mini Totem** (`events.yml`, `totem:`): a column of 5 blocks (3 for the Mini Totem), bedrock between runs and quartz during one. Teams break it with a sword, one hit a block; every block a team breaks turns to bedrock, and the first team to break the whole column wins — but **a block broken by any other team starts it over**, every block quartz again (`rival-break`, `RESET` or `RESET_AND_START`). Set up with `/events create totem|minitotem <id>` and `/events setblock <id>`; `%totem_line%` on the scoreboard, a hologram, a Lunar waypoint and `points.per-totem-win` in `teams.yml`. *Existing servers: copy the `totem:` section of `events.yml`, `per-totem-win` of `teams.yml` and the `%totem_line%` rows of `ui.yml` from the jar.*
...
### Fixed
- **Waypoints were in the player's list but nowhere in the world**: Apollo leaves a waypoint's beam and block outline off unless the server asks for them, and the plugin never did. Both are on now (`apollo.yml`, `waypoints.show-beam`, `highlight-block`).
- **Waypoints and nametags stayed gone after `/apollo reload`**: Apollo drops everything the server sent and tells nobody, and the plugin only ever sent what had changed. It now sends the lot again every `resend-seconds` (30, `apollo.yml`).
...
- **Every running event has a waypoint, and its height is a setting** (`apollo.yml`, `waypoints.event-height`, `0` as shipped — on the objective itself). The King's waypoint follows the player and is never raised.
