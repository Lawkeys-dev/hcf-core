...
- **Two tab lists** (`ui.yml`, `tablist.style`): **the HCF grid** — four columns of twenty cells in place of the player list: player info and location, team and members, server and events, top teams, every cell yours to write, **with heads** (yours, your members', the top teams' leaders, icons beside the headings, any account's skin) — and **the classic list**, each player written with their **LuckPerms prefix and suffix** and ordered by rank, kills or name. `auto`, as shipped, shows the grid on an HCF server and the list on a kitmap. The grid needs the **PacketEvents** plugin (2.13.0+), a new optional dependency; without it the classic list is shown. The Theme Builder previews and writes both.
