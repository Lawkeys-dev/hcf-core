...
- **Where no ability works**: a safe zone (spawn), a Citadel's claim, **the territory of an event** (its land, drawn with `/events claim`) — for as long as it runs, or at all times, as `abilities.yml` or the event itself (`disable-abilities`) says, the zone of a running event (the area held or stood in), the Nether, the End — and the warzone if you switch it on.
