...
- **Where no ability works**: a safe zone (spawn), a Citadel's claim, **the territory of a running event** (its land, drawn with `/events claim`, for as long as it runs — an event can allow them with `disable-abilities: false`), the zone of a running event (the area held or stood in), the Nether, the End — and the warzone if you switch it on.
