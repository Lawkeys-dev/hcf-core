...
- To make the Bard the **only source of Strength**, forbid Strength potions: `strength: 0` under `potions.caps` in `limiters.yml` ([potion caps](items.md#enchantment-and-potion-caps)). A Bard's effects are not potions: the cap never touches them.
