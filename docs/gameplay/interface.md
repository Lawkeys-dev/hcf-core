...
- **`hcf` — the HCF grid.** Four columns of twenty cells in place of the player list: your stats and where you stand, your team and its members (online first, `**` the leader, `*` the co-leaders), the server and running events, the teams with most points. **Every cell is yours to write**, with the scoreboard's placeholders and the tab list's own. A cell whose placeholders all come out empty is left blank, so the grid never shifts. **Each cell has a head**: your own beside your stats, each member's beside their name, each top team's leader beside it, and icons beside the headings — any Minecraft account's skin, the `MHF_` icons (a chest, a question mark, an arrow...) as shipped. Heads are asked of Mojang once and remembered; the rest show one plain default head. **Needs the [PacketEvents](../server/integrations.md#packetevents) plugin**: the game has no other way to show a line that is not a player. Without it, the classic list is shown, and the console says so.
