...
`/team here` (alias `claiminfo`) tells who owns the land you stand on, the claim's size and corners, whether it is protected or raidable, and the team's territory. `/team map` marks the territory around you **in the world**: a column on each corner of every **team's** claim within `map.pillars.radius-chunks` (4) chunks — server land is left out unless `include-system-claims` says otherwise — from the ground to layer `map.pillars.top-y` (128), one ore or block of ore per team, drawn at random each time and named in the chat under the map. The columns are sent to you alone — nothing is placed — and go after `map.pillars.seconds` (20), or as soon as you type `/team map` again: the command is the switch, and once more brings them back. The chat names the block of each team, so a pillar tells you whose land you are looking at. `/team map chat` draws the old grid instead: one character per `map.chat.cell-blocks` (8) blocks, coloured by relation — yours, an ally's, an enemy's, server land, warzone, wilderness — and `map.style` says which of the two `/team map` gives by default.
...
- the players it refuses see a **wall of red glass** around the claim, from the ground to layer 128, appearing as they come within `lock.wall.show-within-blocks` (5) blocks of it, the part of it near them, sent to each of them alone as the wand's columns are — never placed in the world. Members see nothing. It is `lock.wall` in `claims.yml`: the block, its height, how much of the border is drawn, or `enabled: false` for no wall at all.
