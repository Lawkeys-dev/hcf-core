...
- the players it refuses see a **wall of red glass** around the claim, from the ground to layer 128, the part of it near them, sent to each of them alone as the wand's columns are — never placed in the world. Members see nothing. It is `lock.wall` in `claims.yml`: the block, its height, how much of the border is drawn, or `enabled: false` for no wall at all.
