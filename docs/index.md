...
-   :material-flag-variant:{ .lg .middle } __Events__
    ...
    KOTH, Citadel, Conquest, Kill the King, DTC, Last Break, Slide, Totem and Mini Totem, on daily schedules, with holograms above every zone. Mountains that refill on a clock. SOTW, EOTW and the Purge to open and close the map.
...
-   :material-treasure-chest:{ .lg .middle } __Kits and items__
    ...
    Kits built from an inventory, refill signs, a layout editor, and 42 partner items built in — Switcher, Ninja Track, Pocket Bard, Grappling Hook… Custom enchants, effect commands such as `/speed`, enchantment, potion and effect caps, block limits per claim, and a crowbar for End portal frames.
