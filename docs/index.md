...
-   :material-treasure-chest:{ .lg .middle } __Kits and items__
    ...
    Kits built from an inventory, refill signs, a layout editor and partner items. Custom enchants, enchantment and potion caps, block limits per claim, and a crowbar for End portal frames.
...
!!! success "Tested rules"
    Every rule engine is plain Java, covered by more than 900 unit tests — including real round trips on SQLite and checks against MySQL's reserved words — and tried in game on Paper 26.2 with SQLite, MySQL 8.4, LuckPerms, Vault and Lunar Client.
