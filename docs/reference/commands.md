...
| `/bounty clear <player>` | | Remove a bounty — the money is not given back | `hcfcore.economy.bounty.admin` |
...
| `/speed` · `/strength` · `/resistance` · `/regeneration` · `/jumpboost` · `/fireresistance` · `/invisibility` · `/haste` · `/nightvision` | `/sp` · `/str` · `/res` · `/regen` · `/jb` · `/fres`, `/fr` · `/invis`, `/invi` · `/hst` · `/nv` | Give yourself the effect until you die; again to take it off. The commands of [`effect-commands.yml`](configuration/effect-commands.md) — yours may differ | `hcfcore.effect.<command>` |
