...
public record StaffSettings(
    ...
    public static StaffSettings defaults() {
        return new StaffSettings(
                ...
                new StaffChatRules(true, "{primary}[SC] {secondary}%player%{muted}: {secondary}%message%"),
                new BroadcastRules("{dark}[{error}Broadcast{dark}] {secondary}%message%", 100),
    ...
    private static StaffToolbar defaultToolbar() {
        return StaffToolbar.of(List.of(
                new ToolbarItem(0, "COMPASS", "{secondary}&lPlayer Tracker",
                        List.of("{muted}Teleports you to the nearest player."),
                        "staff tpnearest", false),
                new ToolbarItem(1, "ENDER_EYE", "{primary}&lRandom Teleport",
                        List.of("{muted}Teleports you to a random player."),
                        "staff randomtp", false),
                new ToolbarItem(2, "BOOK", "{warning}&lInspect",
                        List.of("{muted}Right-click a player to see", "{muted}what they are carrying."),
                        "invsee %player%", true),
                new ToolbarItem(3, "PACKED_ICE", "{secondary}&lFreeze",
                        List.of("{muted}Right-click a player to hold", "{muted}them for a check."),
                        "freeze %player%", true),
                new ToolbarItem(7, "GRAY_DYE", "{success}&lVanish",
                        List.of("{muted}Toggles whether players can see you.", "{muted}Grey: seen. Green: vanished."),
                        "vanish", false, "LIME_DYE"),
                new ToolbarItem(8, "BARRIER", "{error}&lLeave Staff Mode",
                        List.of("{muted}Gives your own items back."),
                        "staff", false)),
                warning -> {
